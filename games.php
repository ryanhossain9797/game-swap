...
<div class="container" id="posts">
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while($row = mysqli_fetch_assoc($result)) {
            echo "<div id='post'>
                <h1>" . $row["gamename"]. "</h1><br>
                <form action='gameinfo.php' method='post'>
                    <div class='form-group'>
                        <input type='hidden' class='form-control' name='gamename' id='gamename' value=\"".$row["gamename"]."\">
                    </div>  
                    <button type='submit' class='btn btn-success'>Game Details</button>
                </form>
            </div>";
        }
    } else {
